#!/usr/bin/env python3
"""
Mission worker node that drives the rover back to the base dome coordinate.

The node is gated by the central mission FSM and only publishes /cmd_vel while
the global state is DOME_RETURN.
"""
...
class DomeReturnNavigator(Node):
    def __init__(self):
        super().__init__('dome_return_navigator')
        ...
        self.fsm_client = self.create_client(Trigger, 'mission/complete_dome_return')
        ...
        self.get_logger().info(
            f"DOME RETURN NAVIGATOR INITIALIZED. Target: x={TARGET_X:.2f}, "
    ...
    def odom_callback(self, msg):
        if self.global_mission_state != 'DOME_RETURN':
            return
        ...
        if distance <= ARRIVAL_TOLERANCE:
            ...
            self.get_logger().info(
                f"Dome return target reached. Distance: {distance:.2f}m",
        ...
        self.cmd_pub.publish(twist)
        self.get_logger().info(
            f"Returning to dome. Distance: {distance:.2f}m | "
    ...
    def notify_mission_control_complete(self):
        ...
        if not self.fsm_client.service_is_ready():
            self.get_logger().info("Waiting for Mission Manager dome-return service...")
            self.is_finished_reported = False
            return
    ...
    def fsm_response_callback(self, future):
        try:
            res = future.result()
            if res.success:
                self.get_logger().info("SUCCESS: Central FSM acknowledged dome return complete.")
            else:
                self.get_logger().error(f"FSM rejected dome-return completion: {res.message}")
                self.is_finished_reported = False
