#!/usr/bin/env python3
"""
Mission worker node that drives the rover to a preassigned astronaut coordinate.

The node is gated by the central mission FSM and only publishes /cmd_vel while
the global state is ASTRONAUT_SEARCH.
"""
...
# Hardcoded /odom-frame target. Defaults to the astronaut pose in mars.world.sdf.
TARGET_X = -14.0
TARGET_Y = 16.0
ARRIVAL_TOLERANCE = 0.5


class AstronautSearcher(Node):
    def __init__(self):
        super().__init__('astronaut_searcher')
        ...
        self.fsm_client = self.create_client(Trigger, 'mission/complete_astronaut_search')
        ...
        self.get_logger().info(
            f"ASTRONAUT SEARCHER INITIALIZED. Target: x={TARGET_X:.2f}, "
    ...
    def odom_callback(self, msg):
        if self.global_mission_state != 'ASTRONAUT_SEARCH':
            return

        curr_x = msg.pose.pose.position.x
        curr_y = msg.pose.pose.position.y
        curr_yaw = self.quaternion_to_yaw(msg.pose.pose.orientation)
        ...
        if distance <= ARRIVAL_TOLERANCE:
            ...
            self.get_logger().info(
                f"Astronaut target reached. Distance: {distance:.2f}m",
        ...
        self.cmd_pub.publish(twist)
        self.get_logger().info(
            f"Driving to astronaut target. Distance: {distance:.2f}m | "
    ...
    def notify_mission_control_complete(self):
        ...
        if not self.fsm_client.service_is_ready():
            self.get_logger().info("Waiting for Mission Manager astronaut-search service...")
            self.is_finished_reported = False
            return
    ...
    def fsm_response_callback(self, future):
        try:
            res = future.result()
            if res.success:
                self.get_logger().info("SUCCESS: Central FSM acknowledged astronaut search complete.")
            else:
                self.get_logger().error(f"FSM rejected astronaut-search completion: {res.message}")
                self.is_finished_reported = False
